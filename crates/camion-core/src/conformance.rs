...
const CONTENTS: &[u8] = b"Camion carries files.";
...
impl<'a> Conformance<'a> {
    ...
    async fn ranges_return_a_slice(&self) -> Result<()> {
        ...
        let tail = self
            .provider
            .read(&path, Some(ByteRange::from(7)))
            .await?
            .collect()
            .await?;
        expect(tail == CONTENTS[7..], "an open-ended range returned the wrong bytes")?;

        let middle = self
            .provider
            .read(&path, Some(ByteRange::new(7, 7)))
            .await?
            .collect()
            .await?;
        expect(middle == CONTENTS[7..14], "a bounded range returned the wrong bytes")?;
