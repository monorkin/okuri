...
use okuri_engine::secrets::{InMemory, SecretStore};
...
fn a_destination_that_needs_a_password_asks_for_one() {
    ...
    engine.send(Command::Connect {
        ...
        connection: Box::new(Connection::new(
            ...
            Destination::Ftp(okuri_providers::destination::Ftp {
    ...
    assert!(matches!(
        asked.question,
        okuri_engine::Question::Password { .. }
    ));
...
fn dropping_onto_an_existing_file(
    ...
    assert_eq!(
        asked.question,
        okuri_engine::Question::Overwrite { name: "README.md".to_owned() }
    );
...
fn a_file_dropped_on_another_connection_is_carried_across() {
    ...
    assert_eq!(queued.direction, okuri_engine::transfer::Direction::Between);
    assert_eq!(queued.name, "README.md");
    assert_eq!(queued.total, Some(12));
