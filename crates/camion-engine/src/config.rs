...
impl Connections {
    pub fn default_path() -> Option<PathBuf> {
        Some(config_home()?.join("camion/connections.toml"))
    }
...
mod tests {
    use super::*;
    use camion_providers::destination::{Sftp, SshCredential, S3, S3Preset};
    ...
    fn connections_round_trip_through_the_file() {
        ...
        let path = directory.path().join("camion/connections.toml");
