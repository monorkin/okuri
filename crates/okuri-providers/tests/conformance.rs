...
use okuri_core::conformance::Conformance;
use okuri_core::{RemotePath, Serve};
use okuri_providers::destination::{Sftp, SshCredential};
use okuri_providers::sftp::SftpProvider;
use okuri_providers::trust::TrustEverything;
use okuri_providers::Secret;

#[tokio::test]
#[ignore = "needs test/compose.yaml"]
async fn azure_conforms() {
    use okuri_providers::azure::AzureProvider;
    use okuri_providers::destination::Azure;
...
async fn ftp_conforms() {
    use okuri_providers::destination::Ftp;
    use okuri_providers::ftp::FtpProvider;

    let config = Ftp {
        host: "localhost".to_owned(),
        port: 2121,
        username: "okuri".to_owned(),
        // The test server has no certificate. Okuri defaults to explicit FTPS everywhere
    ...
    let provider = FtpProvider::connect(&config, &Secret::Password("okuri".to_owned()))
        .await
...
async fn webdav_conforms() {
    use okuri_providers::destination::WebDav;
    use okuri_providers::webdav::WebDavProvider;
    ...
    let provider = WebDavProvider::connect(&config, &Secret::Password("okuri".to_owned()))
        .await
...
async fn s3_conforms() {
    use okuri_providers::destination::{S3Preset, S3};
    use okuri_providers::s3::S3Provider;

    let config = S3 {
        bucket: "okuri-test".to_owned(),
...
async fn sftp_says_what_it_tried_to_sign_in_with() {
    ...
    assert!(said.contains("okuri was not accepted by localhost"), "{said}");
...
async fn sftp_reports_where_its_root_sits_on_the_server() {
    ...
    let provider = SftpProvider::connect(
        &config,
        &Secret::Password("okuri".to_owned()),
...
async fn s3_says_who_can_read_a_file_and_signs_a_link_to_it() {
    use okuri_core::{Provider, ProviderExt, Visibility};
    use okuri_providers::destination::{S3Preset, S3};
    use okuri_providers::s3::S3Provider;
    ...
    let provider = S3Provider::connect(
        &config,
        &Secret::KeyPair { id: "okuri".to_owned(), secret: "okuri-secret".to_owned() },
    )
    .await
    .expect("the compose MinIO to be running");
    ...
    assert_eq!(
        ...
        "http://localhost:9000/okuri-test/shared.txt"
    );
...
async fn sftp_changes_a_files_mode() {
    use okuri_core::{Permissions, Provider, ProviderExt};
    use okuri_providers::destination::{SshCredential, Sftp};
    use okuri_providers::sftp::SftpProvider;
...
async fn s3_says_how_a_file_is_served_and_stored() {
    use okuri_core::{Provider, ProviderExt};
    use okuri_providers::destination::{S3Preset, S3};
    use okuri_providers::s3::S3Provider;
...
/// A file uploaded through Okuri has to arrive as what it is.
///
...
async fn an_uploaded_file_says_what_it_is() {
    use okuri_core::{ByteStream, Provider, ProviderExt};
    use okuri_providers::destination::{S3Preset, S3};
    use okuri_providers::s3::S3Provider;
