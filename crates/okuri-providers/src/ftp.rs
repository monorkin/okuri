...
/// The oldest protocol Okuri speaks, and the one with the fewest promises.
///
...
impl FtpProvider {
    ...
    pub async fn connect(config: &FtpConfig, secret: &Secret) -> Result<Self> {
        ...
        // Everything Okuri moves is bytes, and a server that helpfully rewrites line endings
...
/// the browser on this desktop uses, rather than a list Okuri carries around itself.
...
mod tests {
    ...
    fn a_unix_listing_line_becomes_an_entry() {
        let file = Listed::from_str("-rw-r--r-- 1 okuri okuri 250000 Aug 26 10:00 harbour.jpg")
            .unwrap();
        ...
        let folder =
            Listed::from_str("drwxr-xr-x 2 okuri okuri 4096 Aug 26 10:00 reports").unwrap();
    ...
    fn a_unix_listing_line_with_a_space_in_the_name_keeps_it() {
        let file =
            Listed::from_str("-rw-r--r-- 1 okuri okuri 12 Aug 26 10:00 last summer.jpg").unwrap();
